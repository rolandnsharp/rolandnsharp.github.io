...
The denoising task is a warmup. The real goal is generation — an oscillator network that can continue a signal, predict what comes next, produce speech from meaning. An LLM that doesn't tokenize language into subword pieces but instead represents meaning as patterns of oscillation — frequency, damping, and phase.

This might sound fanciful. But consider: your brain doesn't tokenize sound. Your cochlea is a bank of damped oscillators (hair cells) tuned to different frequencies. Your auditory cortex processes the oscillator responses. When you speak, your vocal tract synthesizes sound through the same oscillator physics. Perception and production are the same equation at different stages.

Maybe the right architecture for understanding waves was always made of waves.
